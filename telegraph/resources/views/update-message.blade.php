...
@section('content')
    ...
    <form class="" action="{{ route ('form-update-submit', $data->id) }}" method="post">
        @csrf
        <input class="text-success form-control mb-3" type="text" name="name" id="name"
               value="{{ $data->name }}" placeholder="Ваше имя" >
        <input class="text-success form-control mb-3" type="text" name="subject" id=subject"
               value="{{ $data->subject }}" placeholder="Заголовок поста" >
        <input class="text-success form-control mb-3" type="text" name="email"  id="email"
               value="{{ $data->email }}" placeholder="Ваш email">
        <textarea class="text-success form-control mb-4" name="message" id="message">{{ $data->message }}</textarea>
